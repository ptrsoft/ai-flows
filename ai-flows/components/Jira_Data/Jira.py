...
"""
JiraAPIComponent supports the following endpoints:
- Get Issue
- Create Issue
- Add Comment
- Update Issue
...
If you see an error like 'I can only create issues', check your agent/system prompt or UI logic to ensure 'Create Project' is not being filtered out.
"""
...
class JiraAPIComponent(Component):
    ...
    inputs = [
        ...
        MessageTextInput(
            ...
            advanced=True,
            tool_mode=True,
            value='project = "MTD" ORDER BY created DESC',
        ),
        MessageTextInput(
            name="transition_id",
    ...
    def _build_request_data(self, endpoint: str) -> dict:
        ...
        if endpoint == "Get Account ID":
            ...
            if not self.jql or not isinstance(self.jql, str):
                self.log("No valid JQL provided for 'Search Issues'. Using default: project = 'MTD' ORDER BY created DESC")
                self.jql = 'project = "MTD" ORDER BY created DESC'
